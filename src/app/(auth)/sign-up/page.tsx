...
function SignUpContent() {
    ...
    return (
      <div className="mx-auto max-w-md rounded-2xl bg-slate-900 p-8 text-center text-white">
        ...
        <p className="mt-3 text-slate-400">
          Add the Supabase keys from <code className="text-blue-400">.env.example</code> to enable sign up.
        </p>
      </div>
    );
  }
  ...
  const handleEmailSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If user is immediately confirmed / session active
      if (data.session) {
  ...
  return (
    <div className="mx-auto w-full max-w-md rounded-3xl border border-slate-800 bg-slate-900 p-8 text-white shadow-2xl">
      <div className="text-center">
        ...
        <p className="mt-2 text-sm text-slate-400">Join HackMate and start building teams</p>
      </div>
...
export default function SignUpPage() {
  return (
    <Suspense fallback={<div className="text-center text-slate-400">Loading sign up...</div>}>
      <SignUpContent />
    </Suspense>
  );
}
